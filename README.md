...
- ✅ **Vault tab** — list of locally cached assets with friendly names, size, last-modified, sortable headers, optional thumbnails, delete-with-confirm (cascades to clear the chip on the Assets tab). Each row infers its payload kind from the `data/` layout — `asset` (`data/Content/…`), `plugin` (`data/Engine/Plugins/Marketplace/<name>/<name>.uplugin`) or `unknown` — surfaced as a pill next to the name. Asset rows expose an `Add to project` action that opens the merge dialog with the recorded engine version pre-filled. Scan runs in parallel across asset dirs (libuv-pool-backed) and is warmed in the background at app startup, so opening the tab is effectively instant after auth lands.
...
- ✅ **Fab Giveaway tab** — monthly freebies from Fab's `free_content_blade` endpoint, listings-states server check cross-referenced against owned assets, per-card Claim-on-Fab deep link. Optional non-blocking startup toast (`notifyAboutUnclaimedFreebiesOnStartup`) when items are unclaimed; the unread badge on the tab is always visible regardless of the setting. Startup also runs a throttled "auto-refresh" probe that kicks a full library sync in the background when the user has unclaimed leftover, the UID set has changed (new monthly batch), or it's been ≥ 7 days since the last auto-sync.
