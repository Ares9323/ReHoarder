...
- ✅ **Fab Other library sync** — `/i/library/search?source=acquired&asset_formats=…` for non-UE assets (Blender, Maya, FBX, MetaHuman, Unity, etc.). Same full-pagination strategy as the UE library.
...
- ✅ **Multi-account switcher** — keep multiple Epic accounts authenticated at the same time and toggle between them from the top-bar chip. Per-account scoping for `assets`, `asset_tags`, `sync_state`, `downloads`; per-account encrypted tokens (`auth.tokens.<accountId>`); per-account renderer stores reload on switch. The CF-warmup partition cookies are flushed on switch so the next sync's F1-F5 dance can't reuse the previous identity's `fab_sessionid`. Engine installs are stamped with a synthetic `__engine__` account id so they remain visible regardless of which user is active.
