...
- ✅ **Vault tab** — list of locally cached assets with friendly names, size, last-modified, sortable headers, optional thumbnails, delete-with-confirm (cascades to clear the chip on the Assets tab). Scan runs in parallel across asset dirs (libuv-pool-backed) and is warmed in the background at app startup, so opening the tab is effectively instant after auth lands.
...
- 📋 **Curated `ares-recommended` preset bundle** — the preset library still ships only the `empty` starters as of 0.1.2; the curated baselines (BlueprintAssist, GraphEditor distribute, monitor presets, …) land after a manual review pass.
