...
- ✅ **Assets browser** — multi-token search across title + description + seller/creator (tokens can hit different fields, so `laya cry` surfaces `Crystal Cave` by `Laya Design`), source filter, listing-type filter, category filter, "only downloaded" / "only updatable" / "only bookmarked" toggles. Cards show the asset's seller / creator under the title as a clickable link that opens a Fab search filtered by that creator. Bookmarks + per-card context menu (open on Fab, copy id, hide, …). Image-size picker.
